...
document.getElementById("submit-button").addEventListener("click", function () {
  ...
  document.getElementById("submit-button2").addEventListener("click", function () {
	const selectedOption = document.querySelector('input[name="questionCID"]:checked');
	const modal = document.getElementById("modalCID");
	...
	if (selectedOption.value === "A" || selectedOption.value === "B" || selectedOption.value === "C") {
	  modalMessage.textContent = `Justificativa: A opção A é a correta, pois, 
	  sempre que houver dúvida sobre uma solicitação urgente, é essencial confirmar 
	  a autenticidade da informação com fontes confiáveis. Luiz poderia, por exemplo, 
	  ligar diretamente para o gerente de logística`;
	  modal.classList.add("show");
	  document.body.classList.add("modal-active");
	}
